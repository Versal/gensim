...
def get_texts(filename):
    ...
    texts = ((text, lemmatize, title, pageid) for title, text, pageid in extract_pages(bz2.BZ2File(filename), filter_namespaces))
...
def generate_titles(fname_in, fname_out):
    pages = get_texts(fname_in)
    f_out = open(fname_out, 'w')
    count = 0
    while True:
        try:
            f_out.write(pages.next()[1][1].encode('utf-8')+"\n")
            count += 1
            if count % 10000 == 0: print str(count) + " completed"
